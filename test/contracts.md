...
| S-07 | `buildHeader` collapse | when panel would exceed `MAX_SPLASH_ROW_SHARE` of termRows **or** the longest context/skill/extension/prompt name or shortcut hint would not fit → the lists collapse to one counts line; otherwise names appear inline, shortcut section first | README; amended 2026-07-29 (F-16: the `state.skillsExtensionsListed` half was dropped with the flag — the branch is now asserted on rendered output); amended 2026-08-01 (context participates in the fit check and comes first); amended 2026-08-14 (shortcuts and prompts participate in fit check, section order is shortcuts → context → skills → prompts → extensions) |
...
| S-13 | `buildShortcutSection` | renders shortcut hints as a heading row plus wrapped items; keys dimmed, descriptions as text; hints join with ` · `, wrapping only between complete hints; `<= width`; empty hints → `[]` | doc comment + T-05 |

## extensions.ts

| # | Symbol | Claim | Source |
|---|--------|-------|--------|
| E-01 | `discoverLoadedExtensions` agent dir | auto-discovery of `<agentDir>/extensions/`: a directory's package.json `pi.extensions` entries (paths that exist, resolved against the dir) win over `index.ts`/`index.js`; single `.ts`/`.js` files are extensions; a dir with no entry is **not** reported; dotfiles and `node_modules` skipped; `.gitignore`/`.ignore`/`.fdignore` honored with `!` negation and dir patterns; a leading UTF-8 BOM in package.json is stripped before parsing, mirroring pi's core/pi-manifest.js; amended 2026-09-19 | collaborator `collectAutoExtensionEntries`/`resolveExtensionEntries`/`addIgnoreRules` in package-manager.js (ported verbatim) |
...
| D-15 | `getLoadedPrompts` | filters `pi.getCommands()` for `source === "prompt"`, prefixes names with `/`, then sanitizes, deduplicates, and sorts through `uniqueSorted`; no prompt commands → `[]` | doc comment + `uniqueSorted` contract (T-07) |
| D-16 | `getShortcutHints` | returns five compact startup hints: `app.interrupt` → interrupt, `app.clear`/`app.exit` → clear/exit (combined), literal `/` → commands, literal `!` → bash, `app.tools.expand` → more; keys reflect Pi's effective global keybindings via `keyText()` with fallback defaults when `keyText()` returns empty (module isolation in tests, uninitialized keybindings at startup) | doc comment + `keyText()` from pi-coding-agent |
| D-17 | `getLoadedHeaderItems` return shape | return object includes `prompts: string[]` and `shortcuts: ShortcutHint[]` alongside existing `skills`, `extensions`, `context` | doc comment; amended 2026-08-14 (prompts and shortcuts added) |
...
| H-08 | `installHeader` prompt and shortcut seeding | `installHeader` populates `state.loadedPrompts` and `state.loadedShortcuts` from `getLoadedHeaderItems`; prompts appear as `/name` entries; shortcuts reflect Pi's effective keybindings via `keyText()` with fallback defaults; rendered output shows `[shortcuts] N` before `[context]`, and `[prompts] P` before `[extensions]` | plan; added 2026-08-14 |
...
| GA-08 | `render(width)` | menu lines `<= width` for widths 1..200; terminals `<= SHORT_TERMINAL_ROWS` rows drop the spacer above the menu; with a splash above it the menu pads trailing blank rows to sit centered in the rows between the splash and the terminal bottom, reserving no row for the gate's zero-row footer (pi's fullscreen dock gives the footer slot minSize: 0); amended 2026-09-19 (pi 0.86.0) | W + SHORT_TERMINAL_ROWS doc |
